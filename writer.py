...
def to_ordered_df(s: set) -> pd.DataFrame:
    ...
    full_day_df = pd.DataFrame(full_day, columns=['client_oid', 'funds', 'maker_order_id', 'order_id', 'order_type', 'price', 'product_id', 'reason', 'remaining_size', 'sequence', 'side', 'size', 'taker_order_id', 'time', 'trade_id', 'type'])
    return full_day_df
